...
import { parseDeployment, parseUatDocs } from "../services/googleDocsService";
...
const scanUatProdEnv = async (job: Job) => {
  ...
  const promises = deployments.map(async (deployment) => {
    const parsed = parseUatDocs(deployment);
...
const scanUatEnv = async (job: Job) => {
  ...
  const deployments = await GoogleDocumentClient.readUat(doc_id);
